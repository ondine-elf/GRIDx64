...
static void rgb_putpixel(uint32_t x, uint32_t y, uint32_t color) {
...
void rgb_scroll() {
    ...
    for (uint32_t i = 0; i < total_bytes - scroll_bytes; i++) {
        dst[i] = src[i];
    }

    for (uint32_t y = 0; y < RGB_CHAR_HEIGHT; y++) {
        for (uint32_t x = 0; x < fb.width; x++) {
            rgb_putpixel(x, fb.height - RGB_CHAR_HEIGHT + y, 0);
        }
    }

    cursor_x = 0;
    cursor_y = (fb.height / RGB_CHAR_HEIGHT) - 1;
}

static void rgb_drawchar(int x, int y, char c, uint32_t color) {
    for (int row = 0; row < RGB_CHAR_HEIGHT; row++) {
        uint8_t bits = font8x16[(uint8_t)c][row];
        for (int col = 0; col < RGB_CHAR_WIDTH; col++) {
            if (bits & (1 << (7 - col))) {
                rgb_putpixel(x + col, y + row, color);
            }
        }
    }
}
    ...
    if (c == '\n') {
        ...
        for (uint32_t row = 0; row < RGB_CHAR_HEIGHT; row++) {
            for (uint32_t col = 0; col < RGB_CHAR_WIDTH; col++) {
                rgb_putpixel(cursor_x * RGB_CHAR_WIDTH + col, cursor_y * RGB_CHAR_HEIGHT + row, 0);
            }
        }
        return;
    } else if (c >= 32 && c <= 126) {
        rgb_drawchar(cursor_x * RGB_CHAR_WIDTH, cursor_y * RGB_CHAR_HEIGHT, c, RGB_DEFAULT_COLOR);
        cursor_x++;
